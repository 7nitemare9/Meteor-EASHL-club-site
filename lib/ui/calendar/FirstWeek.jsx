    ...
    return (
      <div style={this.style.calWeek}>
        {this.props.week.map((day) => {
          ...
          if (day > 20) {
            return <span key={`lm-day${day}`} style={this.style.calDateOtherMonth}>{day}</span>
          }
          if ( this.hasScheduledEvent(thisday)) {
            let event = this.hasScheduledEvent(thisday);
            if (thisday === today) {
              return (
                <span title={event.description} Click={() => this.goToEvent(event)} key={`day${day}`} style={this.style.calEventToday}>
                  <img src={`/${event.image}`} />
